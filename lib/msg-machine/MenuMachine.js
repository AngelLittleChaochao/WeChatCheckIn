  ...
  var response2 = function(x) {
    try {
      ...
      if (x.in_msg.EventKey && x.in_msg.EventKey[0] === 'mark') {
        var start_date = new Date('2016.10.9');
        var today = new Date();
        days = parseInt((today-start_date)/1000/3600/24);
        var content = const_msg.repl_message.mark_success_curr;
        x.dst_base.type = 'text';
        x.dst_base.content = '你的习惯已经坚持了 ' + days + ' 天! ' + content;
  ...
  var notify_mark = function (source) {
      return source.consume(function(err, x, push, next) {
          if (err) {
              ...
              var current = null;
              // donot get data from redis since redis restart often and lost data
